$(function(){
	...
	/*
		...
		1. 클릭한 인디케이터(엥커a)가 몇번째 
			요소인지를 나타내는 index 번호추출
		
		2. 그 index번호에 해당하는 a(엥커) 요소의 부모에
			.on클래스를 추가한다.

		3. 그 index번호에 해당하는 a(엥커) 요소의 부모의
			형제에 해당하는 다른 li에게서 .on클래스를 제거한다.

		4. index번호에 매칭되는 슬라이드가 보일 수
			있도록 .slides-container를 이동한다.
